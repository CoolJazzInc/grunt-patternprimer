...
module.exports = function(grunt) {
    ...
    var outputPatterns = function(patternFolder, patterns, cb) {
        getSourceFile(function generatePatterns(content) {
            patterns.forEach(function(file) {
                content += '<div class="pattern ' + file.options.title + '"><div class="display">';
    ...
    var handleFiles = function(patternFolder, files, cb) {
        ...
        grunt.log.writeln('handleFiles', patternFolder, files, cb)
    ...
    var walk = function(dir, done) {
        ...
        grunt.log.writeln('Walking dir', dir);

        var results = [];
        fs.readdir(dir, function(err, list) {
            if (err) {
                grunt.log.writeln('Erorr reading dir', err);
                return done(err);
            }
            var i = 0;
            (function next() {
                ...
                grunt.log.writeln('reading file:', file);

                if (!file) {
                    grunt.log.writeln('no more files:', results);
        ...
        walk(patternFolder, function(err, contents) {
            ...
            contents.forEach(function(content) {
                if (content.substr(-5) === '.html') {
                    grunt.log.writeln('adding content from:', content);
                    files.push(content);
                }
            });
